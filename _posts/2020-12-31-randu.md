...
to several minutes and require at least 16 GB of RAM, so if you run my code you
...
and we see that $$c=2^{29}$$. This means that given any starting seed $$S_1$$ we
cannot generate more than $$2^{29}$$ distinct values. This is not much indeed
for current standards. However, let us investigate why seed 12 was much worse
than seed 11. Here is the code:
...
is good when generating floats? Let we will it next.

Before doing this let me just summarize that we have up to this point learned
...
generating a lot of binary numbers:
...
are significant, which we checked using bootstrapping). Note that we could have
easily made the variance too small if we use e.g. $$2^{29}$$ trials (as you can
see we got exactly the same value 100 times --- for sure not producing enough
variance):
...
Bam! Now we understand the reason for the famous RANDU plot that you can see
in [Wikipedia artice][randu] I linked above:
...
But why it is a problem? Let us illustrate it by splitting each dimension into
$$16$$ bins (so we get $$16^3$$ bins in total). Finally we use the DataFrames.jl
package (which I assume everyone was waiting for :), as it was announced in the
introduction and every story teller tries to follow [Чеховское ружьё][tg]
principle):
...
dense regions of points by other bins). This clearly shows that using RANDU
to perform e.g. integration using MonteCarlo simulation in higher dimensions is
not advisable.
