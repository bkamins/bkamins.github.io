...
Apart of using functions such as `select` or `combine` on a `GroupedDataFrame` it is useful to know
...
Sometimes, when iterating a `GroupedDataFrame` we might be interested not only in a data frame per group, but also in a value of grouping variable. This is easily achieved with the `keys` and `pairs` functions (depending if you only want grouping values or both grouping values and data frames):
